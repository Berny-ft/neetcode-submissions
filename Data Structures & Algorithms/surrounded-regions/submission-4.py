class Solution:
    def solve(self, board: List[List[str]]) -> None:
        ...
        def dfs(row,col,group):
            if row > ROWS or row < 0 or col < 0 or col > COLS or (row,col) in visited or board[row][col] == 'X':
                return group

            visited.add( (row,col) )
            group.add( (row,col) )

            down = dfs(row-1,col,group)
            up = dfs(row+1,col,group)
            right = dfs(row,col+1,group)
            left = dfs(row,col-1,group)

            

            return down.union(up.union(right.union(left)))
